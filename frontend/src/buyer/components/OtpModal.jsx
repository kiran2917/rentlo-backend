...
export const OtpModal = ({ onSuccess, onClose, intendedRole = "buyer" }) => {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[var(--bg)]/90 backdrop-blur-xl animate-fade-in"
      role="dialog"
      aria-modal="true"
      aria-labelledby="otp-modal-title"
    >
      <div
        className="relative w-full max-w-md  backdrop-blur-2xl rounded-3xl p-7 sm:p-9 border border-white/50 shadow-2xl shadow-slate-950/30 focus:outline-none overflow-hidden transition-all"
        tabIndex="-1"
      >
        {/* Top Glowing Gradient Accent Bar */}
        <div className="h-1.5 w-full bg-[var(--accent)] absolute top-0 left-0 right-0 shadow-sm" />

        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-5 right-5 w-9 h-9 rounded-full bg-[var(--border-subtle)]/80 hover:bg-[var(--border)] text-[var(--text-muted)] hover:text-[var(--ink)] flex items-center justify-center transition-all duration-200 hover:rotate-90 z-10 cursor-pointer"
        >
          ...
          <div className="flex items-center gap-2 mb-6 pr-8 mt-1">
            {steps.map((s, i) => (
              <React.Fragment key={s}>
                <div className="flex items-center gap-1.5">
                  <div
                    className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
                      i + 1 < otpStep
                        ...
                        : i + 1 === otpStep
                        ? "bg-[var(--accent)] text-white shadow-md shadow-[var(--accent)]/30 ring-4 ring-[var(--accent)]/20"
                  ...
                  <span
                    className={`text-xs font-bold hidden sm:block ${
                      i + 1 === otpStep ? "text-[var(--ink)]" : "text-[var(--text-muted)]"
                    }`}
                  >
                    {s}
                  </span>
                ...
                {i < steps.length - 1 && (
                  <div className="flex-1 h-0.5 mx-2 bg-[var(--border-subtle)] rounded-full" />
                )}
              </React.Fragment>
            ))}
          </div>
        ...
        {/* Icon Header */}
        <div className="w-12 h-12 rounded-2xl bg-[var(--accent)]/100/10 border border-emerald-500/20 text-[var(--accent)] flex items-center justify-center mb-5 shadow-sm">
        ...
        {otpStep === 1 && (
          <>
            <h2
              id="otp-modal-title"
              className="text-2xl font-extrabold text-[var(--ink)] tracking-tight mb-1"
            >
            ...
            <div className="mb-5 space-y-1.5">
              ...
              <div className="flex items-center rounded-2xl border-2 border-[var(--border)] bg-transparent overflow-hidden focus-within:border-[var(--accent)] focus-within:bg-[var(--surface)] focus-within:ring-4 focus-within:ring-[var(--accent)]/30 transition-all">
                <div className="flex items-center gap-1.5 px-3.5 py-3 bg-[var(--border-subtle)] border-r border-[var(--border)] text-[var(--ink)] font-bold text-sm flex-shrink-0">
                  <span>🇮🇳</span>
                  <span>+91</span>
                </div>
                <input
                  ...
                  className="w-full h-12 px-4 text-base font-bold text-[var(--ink)] bg-transparent outline-none placeholder:text-[var(--text-muted)] placeholder:font-medium"
                  onKeyDown={(e) => {
            ...
            {authMode === "password_login" && (
              <div className="mb-5 space-y-1.5">
                ...
                <input
                  ...
                  className="w-full h-12 rounded-2xl px-4 text-sm font-bold text-[var(--ink)] bg-transparent border-2 border-[var(--border)] focus:border-[var(--accent)] focus:bg-[var(--surface)] focus:ring-4 focus:ring-[var(--accent)]/30 outline-none transition-all"
                  onKeyDown={(e) => {
            ...
            <div className="mb-4 flex items-start gap-2.5 p-3 rounded-2xl bg-transparent border border-[var(--border)]">
              <input
                type="checkbox"
                id="dpdp-consent-modal"
                checked={dpdpConsent}
                onChange={(e) => setDpdpConsent(e.target.checked)}
                className="mt-0.5 h-4 w-4 rounded text-[var(--accent)] focus:ring-emerald-500 cursor-pointer"
              />
              <label htmlFor="dpdp-consent-modal" className="text-xs font-medium leading-relaxed text-[var(--text-muted)] cursor-pointer">
                I agree to the <span className="font-extrabold text-[var(--ink)]">Privacy Policy (v1.0)</span> and consent to data processing under DPDP Act protocols.
              </label>
            </div>
            ...
            <button
              ...
              disabled={otpLoading}
              className="w-full h-12 rounded-2xl text-sm font-extrabold bg-[var(--accent)] hover:opacity-90 text-white shadow-lg shadow-[var(--accent)]/30 transition-all duration-200 disabled:opacity-50 hover:-translate-y-0.5 active:scale-98 cursor-pointer mb-4 flex items-center justify-center gap-2"
            >
              {otpLoading ? (
                <>
            ...
            <div className="text-center">
              <button
                ...
                className="text-sm font-bold text-[var(--accent)] hover:text-emerald-700 transition-colors"
              >
                {authMode === "otp_signup" 
        ...
        {requireOtp && otpStep === 2 && (
          <>
            <h2 id="otp-modal-title" className="text-2xl font-extrabold text-[var(--ink)] tracking-tight mb-1">
              Enter Verification Code
            </h2>
            <div className="text-sm text-[var(--text-muted)] font-medium mb-6 space-y-3">
              <div className="flex items-center justify-between">
                <span>Code sent to <strong className="text-[var(--ink)]">{phone}</strong></span>
                <button 
              ...
              {demoCode && (
                <div className="bg-[var(--accent)]/10 border border-[var(--accent)] rounded-xl p-3 flex items-center justify-between shadow-sm">
                  <span className="text-xs font-semibold text-[var(--accent)] flex items-center gap-1.5">
                    ...
                    Dev OTP: <span className="font-extrabold tracking-widest text-[var(--ink)]">{demoCode}</span>
                  </span>
                  <button 
                    onClick={() => setCode(demoCode)}
                    className="text-xs font-extrabold px-3 py-1 bg-[var(--accent)] text-white rounded-lg hover:bg-[var(--accent-soft)] transition-colors shadow-sm cursor-pointer"
                  >
                    Auto-fill
                  </button>
                </div>
            ...
            <div className="mb-5 space-y-1.5">
              ...
              <input
                ...
                placeholder="123456"
                maxLength={6}
                className="w-full h-12 rounded-2xl px-4 text-lg font-black tracking-[0.5em] text-center text-[var(--ink)] bg-transparent border-2 border-[var(--border)] focus:border-[var(--accent)] focus:bg-[var(--surface)] focus:ring-4 focus:ring-[var(--accent)]/30 outline-none transition-all"
                onKeyDown={(e) => {
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setOtpStep(1)}
                className="w-1/3 h-12 rounded-2xl text-sm font-bold text-[var(--ink)] bg-[var(--border-subtle)] hover:bg-[var(--border)] transition-all cursor-pointer"
              >
                Back
              </button>
              <button
                ...
                className="w-2/3 h-12 rounded-2xl text-sm font-extrabold bg-[var(--accent)] hover:bg-slate-900 border border-[var(--accent)] text-white shadow-md transition-all duration-200 disabled:opacity-50 hover:-translate-y-0.5 active:translate-y-0 cursor-pointer flex items-center justify-center gap-2"
              >
        ...
        {otpStep === (requireOtp ? 4 : 3) && (
          <div className="text-center py-4">
            ...
            <h2 className="text-2xl font-extrabold text-[var(--ink)] mb-2">
              Account Banned
            </h2>
            ...
            <button
              onClick={onClose}
              className="w-full h-12 rounded-2xl font-bold bg-[var(--border-subtle)] text-[var(--ink)] hover:bg-[var(--border)] transition-all"
            >
              Close Window
            </button>
          </div>
