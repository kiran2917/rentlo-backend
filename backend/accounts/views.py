...
class LogoutView(APIView):
    ...
    def post(self, request):
        ...
        # Clear cookies for the root path (matching the active login path)
...
class AdminOwnerKYCListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if status_filter == 'all':
            ...
            # Default: pending / submitted awaiting review
            return User.objects.filter(
                (Q(owner_kyc_status='submitted') | (Q(ownership_document_url__isnull=False) & ~Q(ownership_document_url=''))) &
                ~Q(owner_kyc_status='verified') &
                ~Q(owner_kyc_status='rejected')
            ).order_by('-date_joined')
